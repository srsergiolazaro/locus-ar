...
const arReliability = document.getElementById('arReliability');
const arStability = document.getElementById('arStability');
const simScaleEl = document.getElementById('simScale');
const simPosEl = document.getElementById('simPos');
...
async function init() {
    ...
    const compiledData = await compiler.compileImageTargets([
        ...
        if (Math.floor(p) % 25 === 0)
            log(`Compilation progress: ${Math.round(p)}%`);
    });
...
function handleARUpdate(data, markerW, markerH, controller) {
    ...
    if (data.type === 'updateMatrix') {
        ...
        */
        ...
        const activeIds = new Set((screenCoords || []).map((p) => p.id));
...
function drawFeaturePoints(points) {
...
function drawPoints(coords, stabilities) {
...
function positionOverlay(mVT, markerW, markerH, controller) {
