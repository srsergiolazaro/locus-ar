...
\begin{abstract}
Image-based Augmented Reality (AR) systems rely on computationally intensive feature extraction and matching algorithms. Traditional implementations depend on TensorFlow.js for tensor operations and use dense 84-byte descriptors, leading to massive bundle sizes and execution overhead. This paper presents \textbf{Protocol V5.1 (Moonshot)}, an optimized pipeline that completely eliminates TensorFlow and introduces \textbf{128-bit Locality Sensitive Hashing (LSH)} for descriptors. Our evaluation shows that 128-bit binary fingerprints reduce metadata size by \textbf{58\%} while increasing matching speed by \textbf{10-20$\times$} via hardware-accelerated population count (\texttt{popcount}). This approach enables "Instant AR" experiences on low-end devices with near-zero latency and minimal data footprint.
\end{abstract}
...
\subsubsection{Stage 7: 128-bit LSH Binarization}

To achieve the Moonshot goal of ultra-lightweight bundles, we project the high-dimensional FREAK descriptor onto a 128-bit binary space using Locality Sensitive Hashing (LSH). We sample 128 discriminative bit-pairs from the descriptor to create a fingerprint $H \in \{0,1\}^{128}$.

The similarity between two features is then computed using the Hamming distance $d_H$, which is highly efficient on modern CPUs:
\begin{equation}
d_H(H_1, H_2) = \text{popcount}(H_1 \oplus H_2)
\end{equation}
This replaces the $O(N)$ byte-comparison loop with a hardware-level instruction.
...
\begin{table}[h]
\centering
\caption{Comparison between MindAR (V4) and TapTapp AR Protocol V5.1 (Moonshot)}
\label{tab:results}
\begin{tabular}{lccc}
\toprule
\textbf{Metric} & \textbf{V4 Baseline} & \textbf{V5.1 (LSH)} & \textbf{Reduction} \\
\midrule
Metadata (.mind) & 436 KB & \textbf{181 KB} & 58\% \\
Descriptor Size & 84 bytes & \textbf{16 bytes} & 81\% \\
Matching Logic & Byte-Loop & \textbf{Popcount} & 15$\times$ faster \\
Inliers (Stability) & 768 & 717 & -6\% \\
\bottomrule
\end{tabular}
\end{table}
