...
\begin{figure}[ht]
\centering
\includegraphics[width=0.9\columnwidth]{figures/ar_tracking_demo.png}
\caption{Locus AR in action: Real-time image tracking with detected feature points (green dots) and 3D content overlay. HD 1280×960 resolution enables precise tracking.}
\label{fig:ar_demo}
\end{figure}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.95\columnwidth]{figures/jit_compilation.png}
\caption{JIT Compilation Flow: From image URL to ready-to-track .taar binary in under 1 second. No server-side preprocessing required.}
\label{fig:jit}
\end{figure}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.9\columnwidth]{figures/foveal_attention.png}
\caption{Foveal Attention Model: High-resolution processing in the central fovea, progressively lower resolution in peripheral zones. Reduces processed pixels by 83\%.}
\label{fig:foveal}
\end{figure}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.75\columnwidth]{figures/gaussian_pyramid.png}
\caption{Gaussian Pyramid: Multi-octave image representation. Each level is half the resolution with increased blur for scale-invariant feature detection.}
\label{fig:gaussian}
\end{figure}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.8\columnwidth]{figures/dog_extrema.png}
\caption{DoG Scale-Space Cube: Each candidate pixel (red center) is compared against 26 neighbors across three scale levels ($\sigma-1$, $\sigma$, $\sigma+1$) for extrema detection.}
\label{fig:dog}
\end{figure}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.75\columnwidth]{figures/freak_pattern.png}
\caption{FREAK Retinal Sampling Pattern: Bio-inspired descriptor with 43 sampling points. Higher density in the foveal center, sparser in the periphery, mimicking human retina.}
\label{fig:freak}
\end{figure}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.95\columnwidth]{figures/hdc_hypervector.png}
\caption{HDC Transformation: Image features are projected into a compact 128-bit binary hypervector (16 bytes) for ultra-fast similarity comparison via Hamming distance.}
\label{fig:hypervector}
\end{figure}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.85\columnwidth]{figures/similarity_matrix.png}
\caption{HDC Similarity Matrix (100×100 images): Diagonal shows self-similarity (yellow = 100\%), off-diagonal shows low cross-similarity (dark blue $\approx$ 50\%), demonstrating strong discriminative power. Tested on diverse stock images with 44M+ comparisons/second.}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.85\columnwidth]{figures/delaunay_mesh.png}
\caption{Delaunay Triangulation Mesh: 50 vertices connected by triangles enable non-rigid tracking. Arrows show deformation capability for curved surfaces.}
\label{fig:delaunay}
\end{figure}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.9\columnwidth]{figures/mass_spring.png}
\caption{Mass-Spring Relaxation: Vertices (blue nodes) connected by springs. Red=stretched (tension), green=compressed, gray=at rest. Purple arrows show force vectors for mesh optimization.}
...
\begin{table}[ht]
\centering
\caption{MindAR vs Locus AR V11}
\label{tab:results}
\footnotesize
\begin{tabular}{lccc}
\toprule
...
\caption{Performance comparison: Locus AR achieves 23× faster compilation and 200× smaller bundle.}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.95\columnwidth]{figures/performance_chart.png}
\caption{Framework Comparison: Locus AR vs MindAR. TapTapp achieves 24× faster compilation ($\sim$1s vs 24s) and 200× smaller bundle (0.1MB vs 20MB).}
\label{fig:perfchart}
\end{figure}
...
\url{https://github.com/srsergiolazaro/locus-ar}

Published as npm package: \texttt{locus-ar}
