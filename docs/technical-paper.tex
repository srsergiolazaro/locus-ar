...
\begin{abstract}
Image-based Augmented Reality (AR) systems rely on computationally intensive feature extraction algorithms during the compilation phase of image targets. Traditional implementations depend on TensorFlow.js for GPU-accelerated tensor operations, introducing significant overhead and compatibility issues in server-side environments. This paper presents \textbf{DetectorLite}, a novel pure JavaScript implementation of scale-invariant feature detection that completely eliminates TensorFlow dependencies while achieving \textbf{2.9× faster compilation} and \textbf{15\% more feature points} compared to the reference MindAR implementation. Our approach demonstrates that carefully optimized JavaScript can outperform tensor-based frameworks for specific computer vision tasks, particularly in serverless and edge computing scenarios where cold start latency is critical.
\end{abstract}
...
\begin{enumerate}
    ...
    \item Empirical comparison showing 2.9× speedup over TensorFlow-based baseline
    \item Evidence that optimized JavaScript detects 15\% more features
\end{enumerate}
...
\begin{table}[h]
\centering
\caption{Performance comparison between DetectorLite and MindAR baseline}
\label{tab:results}
\begin{tabular}{lccc}
\toprule
\textbf{Metric} & \textbf{DetectorLite} & \textbf{MindAR} & \textbf{Improvement} \\
\midrule
Compilation time & \textbf{0.178s} & 0.523s & 2.9× faster \\
Feature points & \textbf{54} & 47 & +15\% \\
TensorFlow required & No & Yes & Eliminated \\
Cold start time & $\approx$0s & $\approx$1.5s & Eliminated \\
\bottomrule
\end{tabular}
\end{table}
